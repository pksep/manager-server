...
Повторный сеанс с совпавшими нормализованными телефоном и email продолжает
открытый чат клиента. Сеансы посетителя остаются раздельными. Правило повторного
открытия закрытого обращения пока отдельно не определено.
...
| Совпали нормализованные телефон и email | Использовать существующего клиента и его открытый чат; сохранять ответственного, примечание и связь с ЕРП |
| Совпал только один реквизит | Сохранить отдельного клиента и предложить совпадения менеджеру |
...
Уточнение от 14.09.2026: совпадение обоих реквизитов устанавливает связь карточки,
но не подтверждает владение телефоном/email. Клиент видит только сообщения своей
гостевой сессии, включая после переподключения WebSocket и запросов истории.
Обычный ответ менеджера направляется в сессию последнего доставленного в чат
посетительского сообщения. Событие сохраняет идентификатор этой сессии в момент
ответа, поэтому смена активного браузера не перенаправляет уже созданный ответ.
